...
comm.allcommon <- function(x, comm = .mpiopt_get("SPMD.CT", "comm"),
    lazy.check = .mpiopt_get("SPMD.CT", "lazy.check")){
  if(lazy.check){
...
### Internal only.
comm.allcommon.integer <- function(x, comm = .mpiopt_get("SPMD.CT", "comm")){
  x <- as.integer(x)
