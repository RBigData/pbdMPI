...
comm.allcommon <- function(x, comm = .pbdMPIEnv$SPMD.CT$comm,
    lazy.check = .pbdMPIEnv$SPMD.CT$lazy.check){
  if(lazy.check){
...
### Internal only.
comm.allcommon.integer <- function(x, comm = .pbdMPIEnv$SPMD.CT$comm){
  x <- as.integer(x)
