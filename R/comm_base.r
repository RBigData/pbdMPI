...
comm.length <- function(x, comm = .mpiopt_get("SPMD.CT", "comm")){
...
comm.sum <- function(..., na.rm = TRUE, comm = .mpiopt_get("SPMD.CT", "comm")){
  tmp <- sum(..., na.rm = na.rm)
  ret <- allreduce(tmp, op = "sum", comm = comm)
  ret
} # End of comm.sum().

comm.mean <- function(x, na.rm = TRUE, comm = .mpiopt_get("SPMD.CT", "comm")){
  if(! comm.all(is.vector(x), comm = comm)){
    comm.stop("x should be a vector.", comm = comm)
  }
...
comm.var <- function(x, na.rm = TRUE, comm = .mpiopt_get("SPMD.CT", "comm")){
  if(! comm.all(is.vector(x), comm = comm)){
    comm.stop("x should be a vector.", comm = comm)
  }
...
comm.sd <- function(x, na.rm = TRUE, comm = .mpiopt_get("SPMD.CT", "comm")){
