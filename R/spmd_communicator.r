spmd.barrier <- function(comm = .pbdEnv$SPMD.CT$comm){
...
spmd.comm.free <- function(comm = .pbdEnv$SPMD.CT$comm){
...
spmd.finalize <- function(mpi.finalize = .pbdEnv$SPMD.CT$mpi.finalize){
...
spmd.comm.abort <- function(errorcode = 1, comm = .pbdEnv$SPMD.CT$comm){
...
spmd.comm.split <- function(comm = .pbdEnv$SPMD.CT$comm, color = 0L,
    key = 0L, newcomm = .pbdEnv$SPMD.CT$newcomm){
...
spmd.comm.disconnect <- function(comm = .pbdEnv$SPMD.CT$comm){
  if(spmd.comm.size(comm)== 0){
    stop(paste("It seems no members (workers) associated with comm", comm))
  }
...
spmd.comm.connect <- function(port.name,
    info = .pbdEnv$SPMD.CT$info, rank.root = .pbdEnv$SPMD.CT$rank.root,
    comm = .pbdEnv$SPMD.CT$comm, newcomm = .pbdEnv$SPMD.CT$newcomm){
...
spmd.port.open <- function(info = .pbdEnv$SPMD.CT$info){
...
spmd.serv.publish <- function(port.name,
    serv.name = .pbdEnv$SPMD.CT$serv.name,
...
spmd.serv.lookup <- function(serv.name = .pbdEnv$SPMD.CT$serv.name,
    info = .pbdEnv$SPMD.CT$info){
...
spmd.comm.get.parent <- function(comm = .pbdEnv$SPMD.CT$intercomm){
  .Call("spmd_comm_get_parent", as.integer(comm), PACKAGE = "pbdMPI")
} # End of spmd.comm.get.parent().

spmd.intercomm.merge <- function(intercomm = .pbdEnv$SPMD.CT$intercomm,
    high = 0L, comm = .pbdEnv$SPMD.CT$comm){
...
spmd.intercomm.create <- function(local.comm = .pbdEnv$SPMD.CT$comm,
    local.leader = .pbdEnv$SPMD.CT$rank.source,
    peer.comm = .pbdEnv$SPMD.CT$intercomm,
    remote.leader = .pbdEnv$SPMD.CT$rank.dest, tag = .pbdEnv$SPMD.CT$tag,
    newintercomm = .pbdEnv$SPMD.CT$newcomm){
...
spmd.comm.c2f <- function(comm = .pbdEnv$SPMD.CT$comm){
