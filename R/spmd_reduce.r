...
### Default method.
spmd.reduce.default <- function(x, x.buffer = NULL, op = .pbdMPIEnv$SPMD.CT$op,
...
spmd.reduce.integer <- function(x, x.buffer, op = .pbdMPIEnv$SPMD.CT$op,
    rank.dest = .pbdMPIEnv$SPMD.CT$rank.source, comm = .pbdMPIEnv$SPMD.CT$comm){
  ret <- .Call("spmd_reduce_integer", x, x.buffer,
               which(op[1] == .pbdMPIEnv$SPMD.OP),
...
spmd.reduce.double <- function(x, x.buffer, op = .pbdMPIEnv$SPMD.CT$op,
    rank.dest = .pbdMPIEnv$SPMD.CT$rank.source, comm = .pbdMPIEnv$SPMD.CT$comm){
