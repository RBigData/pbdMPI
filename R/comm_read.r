...
comm.read.table <- function(file, header = FALSE, sep = "", quote = "\"'",
    ...
    read.method = .pbdEnv$SPMD.IO$read.method[1],
    balance.method = .pbdEnv$SPMD.IO$balance.method[1],
...
read.table.gbd <- function(file, header = FALSE, sep = "",
  ...
  if(file.size > .pbdEnv$SPMD.IO$max.read.size){
    comm.cat("Caution: file size exceeds .pbdEnv$SPMD.IO$max.read.size.\n",
    ...
    if(file.size < .pbdEnv$SPMD.IO$max.read.size){
      ...
      if(COMM.RANK == 0){
        tmp <- nchar(readLines(con = file,
                               n = .pbdEnv$SPMD.IO$max.test.lines))
