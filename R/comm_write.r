...
comm.write <- function(x, file = "data",
    ...
    append = FALSE, sep = " ", comm = .pbdEnv$SPMD.CT$comm){
...
comm.write.table <- function(x, file = "", append = FALSE,
    ...
    fileEncoding = "", comm = .pbdEnv$SPMD.CT$comm){
  comm.rank <- spmd.comm.rank(comm = comm)
...
comm.write.csv <- function(..., comm = .pbdEnv$SPMD.CT$comm){
...
comm.write.csv2 <- function(..., comm = .pbdEnv$SPMD.CT$comm){
  Call <- match.call(expand.dots = TRUE)
  Call$sep <- ";"
  Call$dec <- ","
  Call$qmethod <- "double"
