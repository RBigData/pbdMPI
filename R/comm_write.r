...
comm.write <- function(x, file = "data",
    ...
    append = FALSE, sep = " ", comm = .mpiopt_get("SPMD.CT", "comm")){
...
comm.write.table <- function(x, file = "", append = FALSE,
    ...
    fileEncoding = "", comm = .mpiopt_get("SPMD.CT", "comm")){
  comm.rank <- spmd.comm.rank(comm = comm)
...
comm.write.csv <- function(..., comm = .mpiopt_get("SPMD.CT", "comm")){
...
comm.write.csv2 <- function(..., comm = .mpiopt_get("SPMD.CT", "comm")){
  Call <- match.call(expand.dots = TRUE)
  Call$sep <- ";"
  Call$dec <- ","
  Call$qmethod <- "double"
