...
comm.range <- function(..., na.rm = FALSE, comm = .pbdEnv$SPMD.CT$comm){
  ret <- range(...)
...
comm.max <- function(..., na.rm = FALSE, comm = .pbdEnv$SPMD.CT$comm){
  ret <- max(..., na.rm = na.rm)
  ret <- max(do.call("c", allgather(ret[1], comm = comm)), na.rm = na.rm)
  ret
} # End of comm.max().

comm.min <- function(..., na.rm = FALSE, comm = .pbdEnv$SPMD.CT$comm){
  ret <- min(..., na.rm = na.rm)
